...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                final Optional<Method> matchedMethod = Arrays.stream(clazz.getDeclaredMethods()).filter(
                        (met) -> {
                            boolean result = met.getName().equals(route.getAction());
                            final Class<?>[] parameterTypes = met.getParameterTypes();
                            final Set<Class<?>> validTypes = Set.of(Map.class, HttpServletRequest.class);
                            result = (result && Arrays.stream(parameterTypes).allMatch(
                                    p -> validTypes.contains(p)
                            ));
                            result = (result && Arrays.stream(parameterTypes).distinct().count() == parameterTypes.length);
                            return result;
                        }
                ).findAny();
        ...
        try {
            ...
            for (int i = 0; i < len; ++i) {
                ...
                final Optional<Object> optional = Stream.of(request, view).filter(type::isInstance).findAny();
                args[i] = optional.orElseThrow(() -> new IllegalStateException("unknown type + " + type));
            }
        ...
        if (Objects.nonNull(requestLang) && requestLang.length() == 2 && requestLang.toLowerCase().equals(requestLang)) {
            request.getSession().setAttribute("lang", requestLang);
        }
        String lang = Objects.toString((String) request.getSession().getAttribute("lang"), "");
